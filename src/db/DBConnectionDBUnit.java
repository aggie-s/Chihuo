...
public class DBConnectionDBUnit extends DatabaseTestCase {
	protected IDatabaseConnection getConnection() throws Exception {
		...
		Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/unittest?", "root", "password");
		return new MySqlConnection(conn, "unittest");
	}

	@Override
	protected IDataSet getDataSet() throws Exception {
		return new FlatXmlDataSetBuilder().build(new FileInputStream("src/db/DBUnitTest.xml"));
	}
	...
	public void testGetVisitedRestaurants() throws Exception {
		DBConnection dbconn = new DBConnection("jdbc:mysql://localhost:3306/unittest?user=root&password=password");
		String userId = "1111";
		Set<String> visited = dbconn.getVisitedRestaurants(userId);
		assertEquals(
				new HashSet<String>(
						Arrays.asList("--qeSYxyn62mMjWvznNTdg", "--zgHBiQpr8H2ZqSdGmguQ", "-1B-DEGkLE1kDj5ENAF2NQ")),
				visited);
		dbconn.close();
	}
	...
	public void testSetVisitedRestaurants() throws Exception {
		...
		Set<String> visited = dbconn.getVisitedRestaurants(userId);
		assertEquals(new HashSet<String>(Arrays.asList("--qeSYxyn62mMjWvznNTdg", "--zgHBiQpr8H2ZqSdGmguQ",
	...
	public void testRecommendRestaurants() throws Exception {
		...
		Set<String> expected = new HashSet<String>(Arrays.asList("--jFTZmywe7StuZ2hEjxyA", "-1BzcQK-HDA6LVOThHMpsw",
				"--UE_y6auTgq3FXlvUMkbw", "--Y_2lDOtVDioX5bwF6GIw", "-1ERbsOf9XOC9wGbZYFr7g", "-024YEtnIsPQCrMSHCKLQw",
				"--BlvDO_RG2yElKu9XA1_g", "-0tJ4FwcNtJjil-ZUTy30Q", "-0lOuL7RkZQnjAl96dXTvA",
